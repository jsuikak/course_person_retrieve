...
DEFAULT_DOC_PATH = REPO_ROOT / "docs" / "融合检索以及Celeb-reID基准测试.md"
OFFICIAL_SPLITS = {
    "Celeb-reID": {
...
def _markdown_algorithm_section(dataset_name: str, config: FusionFeatureConfig) -> str:
    text = r"""## 算法与向量检索方案
...
2. OSNet-only：不再跑 YOLO，直接把 __DATASET_NAME__ 的裁剪行人图像输入 OSNet，得到整体行人特征。
3. ArcFace+OSNet fusion：使用固定权重加权拼接，默认 `face_weight=__FACE_WEIGHT__`，`person_weight=__PERSON_WEIGHT__`。
...
$$w_f=__FACE_WEIGHT__, \quad w_p=__PERSON_WEIGHT__$$
...
$$S_{q,g}=__FACE_WEIGHT__\mathbf{f}_q^\top\mathbf{f}_g + __PERSON_WEIGHT__\mathbf{p}_q^\top\mathbf{p}_g$$
...
"""
    return (
        text.replace("__DATASET_NAME__", dataset_name)
        .replace("__FACE_WEIGHT__", str(config.face_weight))
        .replace("__PERSON_WEIGHT__", str(config.person_weight))
    )
...
def _write_markdown_report(
    ...
    content = rf"""# 融合检索以及 {dataset_name} 基准测试
...
{_markdown_algorithm_section(dataset_name, config)}
